...
###########################################################
###########################################################
##Programme pour que le bot joue


def bot(num_jouer_courant):
    colonne = random.randint(0, 6)  #choisi une colonne au hasard
    if grille_pleine == True:    #vérifie que la grille soit pleine, si c'est le cas une égalité est déclencher
        affiche_grille() 
        print("La grille est pleine, c'est une égalité:\n") 
        
        return "tie"        #renvoie tie pour symboliser une égaliter car la grille est pleine et donc le jeu est finie
    else:
        if colonne_pleine(colonne) == True:   #verfie si la colonne est pleine, si c'est le cas le programme recomence pour rechoisire une autre colonne
            bot(num_jouer_courant)
        else:
            time.sleep(0.5)   #faisons attendre le bot, on le mets ici pour que si le bot doit rechosire la colonne il ne doit pas attendre 1 voire 1.5 seconde puisque il refait la fonction sleep
            joue_jeton(num_joueur_courant, colonne)
            print("")
            print("Le bot a joué sur la colonne:  ",colonne, " \n")
            if Recherche_si_victoire(num_jouer_courant) == True:
...
Il y en a pas, ca a marché du premier coup, saud que j'ai mis random.randint(0, 7) et que c'est out of index (je suis aussi trés surpris que ca marche aussi bien)

Pour le test que j'ai fait j'ai just jouez au jeux et j'ai laisser le bot avoir une victoire pour vérifier si la fonction victoire marché bien, 
ainsi que de remplire une colonne entiere pour vérifier que le bot recomence bien si il y a une colonne pleine
...
while partie_terminee==False:
    ...
    if num_joueur_courant == 0:
        ...
        if bot(num_joueur_courant) == "end":   #nous avons une fonction pour que le bot joue  si il renvoie end cela veut dire que le programme s'arrete
